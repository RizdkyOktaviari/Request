  ...
  if($gambar_produk != "") {
    ...
    $nama_gambar_baru = $_SESSION["user"].'-'.date("l_jS_n-h_i_s ").'-'.$gambar_produk; //menggabungkan angka acak dengan nama file sebenarnya
          ...
          if(in_array($ekstensi, $ekstensi_diperbolehkan) === true)  {
                  move_uploaded_file($file_tmp, '../../Dashboard/add-item/gambar/'.$nama_gambar_baru); //memindah file gambar ke folder gambar
                    ...
                    if(!$result){
                         ...
                         echo "<script>alert('Data berhasil diubah.');window.location='index.php?id=$id';</script>";
                       }
     ...
     $query = "INSERT INTO `tabel_permintaan`(`Judul`, `label`, `imbalan`, `tgl_permintaan`, `deskripsi`, `gambar`, `status`)
     VALUES ('$judul','$tipe','$imbalan','$tgl','$deksripsi',null,'aktif')";
                    $result = mysqli_query($conn, $query);
                    // periska query apakah ada error
                    if(!$result){
                        die ("Query gagal dijalankan: ".mysqli_errno($conn).
